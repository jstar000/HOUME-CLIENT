...
export const useDetectionPrefetch = () => {
  const enabled = IS_CLIENT_DETECTION_MODE;
  ...
  const executePrefetch = useCallback(
    ...
    [enabled, error, isLoading, processAndStore, runInference]
  );
  ...
  }, [enabled, executePrefetch, runWithSemaphore]);
  ...
  const prefetchDetection = useCallback(
    ...
    [enabled, executePrefetch, runWithSemaphore, scheduleBackgroundPrefetch]
  );
